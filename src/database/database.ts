...
export const deleteGroup = async (id: string): Promise<void> => {
  ...
  await getDb().runAsync(
    "UPDATE broadcast_lists SET group_id = NULL WHERE group_id = ?",
    id,
  );
...
// ============ Broadcast Lists ============
export const addBroadcastList = async (
  id: string,
  name: string,
...
export const updateBroadcastListName = async (
  id: string,
  name: string,
): Promise<void> => {
  await getDb().runAsync(
    "UPDATE broadcast_lists SET name = ? WHERE id = ?",
    name,
    id,
  );
};

export const assignListToGroup = async (
  listId: string,
  groupId: string | null,
): Promise<void> => {
  await getDb().runAsync(
    "UPDATE broadcast_lists SET group_id = ? WHERE id = ?",
    groupId,
    listId,
  );
  ...
  if (groupId) {
    await getDb().runAsync(
      "DELETE FROM ungrouped_notifications WHERE list_id = ?",
      listId,
    );
  }
};
...
export const getListsByGroup = async (
  groupId: string,
...
export const searchListContacts = async (
  listId: string,
  query: string,
): Promise<ListContact[]> => {
  const results = await getDb().getAllAsync<ListContact>(
...
export const setActiveGroupId = async (
  groupId: string | null,
): Promise<void> => {
...
export const getAppSettings = async (): Promise<{
  ...
  const floatingBubbleEnabled =
    (await getSetting("floating_bubble_enabled")) === "true";
  const pushNotificationsEnabled =
    (await getSetting("push_notifications_enabled")) === "true";
...
export const updateAppSettings = async (
  ...
  if (settings.floatingBubbleEnabled !== undefined) {
    await setSetting(
      ...
      settings.floatingBubbleEnabled.toString(),
    );
  }
  if (settings.pushNotificationsEnabled !== undefined) {
    await setSetting(
      "push_notifications_enabled",
      settings.pushNotificationsEnabled.toString(),
    );
  }
...
export const addUngroupedNotification = async (
  listId: string,
  listName: string,
  contactCount: number,
): Promise<void> => {
...
export const getUngroupedNotifications = async (): Promise<
  UngroupedListNotification[]
> => {
...
export const removeUngroupedNotification = async (
  listId: string,
): Promise<void> => {
  await getDb().runAsync(
    "DELETE FROM ungrouped_notifications WHERE list_id = ?",
...
// ============ Sync Helpers ============
export const syncBroadcastLists = async (
  lists: { id: string; name: string; contacts: ListContact[] }[],
): Promise<void> => {
  for (const list of lists) {
    // Check if list already exists
    const existing = await getDb().getFirstAsync<{ id: string }>(
      "SELECT id FROM broadcast_lists WHERE id = ?",
      list.id,
    );
